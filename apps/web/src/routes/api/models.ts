...
if (TRUST_PROXY_MODE === "true") {
	console.warn(
		"[Models API] TRUST_PROXY=true is deprecated and insecure. " +
		"The x-forwarded-for header is user-controlled and can be spoofed to bypass rate limiting. " +
		"Use TRUST_PROXY=cloudflare or TRUST_PROXY=vercel for production deployments. " +
		"Falling back to platform-specific headers only; raw x-forwarded-for will NOT be trusted.",
	);
}
...
function getClientIp(request: Request): string | null {
	...
	if (TRUST_PROXY_MODE === "true") {
		// TRUST_PROXY=true is deprecated. Only trust platform-specific headers
		// that are set/overwritten by the edge proxy and are harder to spoof.
		// Do NOT fall back to x-forwarded-for as it is user-controlled.
		...
		// x-forwarded-for is intentionally NOT checked here because it can be
		// spoofed by clients to bypass rate limiting. If no platform-specific
		// header is available, return null to fail closed.
		return null;
	}
